...
export async function addCompositePolicy(
  ...
  policies: number[] | undefined | null
): Promise<ClientResponse<string>> {
  const fields: any[] = [shopId, expirationDate, subject, operator, policies];
  if (!checkInput(fields)) return Promise.reject();
...
export async function addCompositeRule(
  ...
  const fields: any[] = [shopId, operator, rules];
  if (!checkInput(fields)) return Promise.reject();
...
export async function getActiveMembers(): Promise<ClientResponse<Member[]>> {
