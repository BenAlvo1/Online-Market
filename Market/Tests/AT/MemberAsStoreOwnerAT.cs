...
namespace Market.AT
{
    ...
    public class MemberAsStoreOwnerAT
    {
        ...
        [TestMethod]
        public void ChangePurchasePolicy_GoodCase()
        {
            Assert.IsTrue(proxy.EnterAsGuest(sessionID));
            Assert.IsTrue(proxy.Register(sessionID, "uniqueUsername1", "password"));
            Assert.IsTrue(proxy.Login(sessionID, "uniqueUsername1", "password"));
            int shopID = 1;
            Assert.IsTrue(proxy.createShop(sessionID, "A dummy shop"));
            Assert.IsTrue(proxy.AddProduct(sessionID, shopID, "Apple", "green apple", productPrice, productQuantity, "Food", new List<string>()));
            Assert.IsTrue(proxy.AddSimpleRule(sessionID, shopID, "Food"));
            int ruleId = 1;
            Assert.IsTrue(proxy.AddPurchasePolicy(sessionID, shopID, "01/01/2030", "Food", ruleId));
            Assert.IsTrue(proxy.UpdateRuleSubject(sessionID, shopID, ruleId, "Pockemon"));

        }

        [TestMethod]
        public void ChangePurchasePolicy_BadCase()
        {
            Assert.IsTrue(proxy.EnterAsGuest(sessionID));
            Assert.IsTrue(proxy.Register(sessionID, "uniqueUsername1", "password"));
            Assert.IsTrue(proxy.Login(sessionID, "uniqueUsername1", "password"));
            ...
            int ruleId = 1;
            Assert.IsTrue(proxy.AddPurchasePolicy(sessionID, shopID, "01/01/2030", "Food", ruleId));
            Assert.IsFalse(proxy.UpdateRuleSubject(sessionID, shopID, ruleId, "null category"));
        }
