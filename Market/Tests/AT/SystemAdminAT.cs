...
namespace Market.AT
{
    ...
    public class SystemAdminAT
    {
        ...
        {
            List<SMember> members = proxy.GetAllMembers(ADMIN_SESSION_ID);
            ...
            foreach (SMember member in members)
            {
                Assert.IsTrue(names.Contains(member.Username));
            }
        }
        ...
        public void GetActiveMembersInfoSuccess()
        {
            ...
            List<SMember> members = proxy.GetActiveMembers(ADMIN_SESSION_ID);
            Assert.IsNotNull(members);
