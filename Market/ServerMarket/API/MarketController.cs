...
namespace ServerMarket.API
{
    ...
    public class MarketController : ControllerBase
    {
        ...
        */
        ...
        {
            Response response = await Task.Run(() => service.AddCompositePolicy(request.SessionId, request.ShopID, request.Expirationdate, request.Subject, request.Operator, request.Policies));
            if (response.ErrorOccured)
            {
                return BadRequest(ServerResponse<string>.sendBadResponse(response.ErrorMessage));
            }
        ...
        public ActionResult<Response<List<SMember>>> GetActiveMembers([FromBody] GetActiveMembersRequest request)
        {
            Response<List<SMember>> response = service.GetActiveMembers(request.SessionId);
            if (response.ErrorOccured)
            {
                return BadRequest(ServerResponse<string>.sendBadResponse(response.ErrorMessage));
            }
            ...
            {
                return Ok(ServerResponse<List<SMember>>.sendOkResponse(response.Value));
            }
        }

        [HttpPost]
        [Route("get-all-members")]
        public ActionResult<Response<List<SMember>>> GetAllMembers([FromBody] GetAllMembersRequest request)
        {
            Response<List<SMember>> response = service.GetAllMembers(request.SessionId);
            if (response.ErrorOccured)
            {
                return BadRequest(ServerResponse<string>.sendBadResponse(response.ErrorMessage));
            }
