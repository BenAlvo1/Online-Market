...
namespace Market.RepoLayer
{
    public class MemberRepo : IRepo<Member>
    {
        ...
        public List<Member> GetOwners(int shopId)
        {
            lock (MarketContext.MarketContextLock)
            {
                List<int> ownersIds = MarketContext.GetInstance().Appointments
                    .Where<AppointmentDTO>((a) => a.ShopId == shopId && (a.Role == "Owner" || a.Role == "Founder"))
                    .Select((a) => a.MemberId).ToList<int>();
