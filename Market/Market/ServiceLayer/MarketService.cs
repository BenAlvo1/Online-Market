...
namespace Market.ServiceLayer
{
    ...
    public class MarketService : IMarket
    {
        ...
        public Response<List<SMember>> GetAllMembers(string sessionID)
        {
            try
            {
                ...
                {
                    sMembers.Add(new SMember(member));
                }
                _logger.Log($"User with session ID {sessionID} accessed GetAllMembers method");
                return Response<List<SMember>>.FromValue(sMembers);
            }
            catch (Exception e)
            {
                _logger.Error($"Error occurred in GetAllMembers method for session ID {sessionID}: {e.Message}");
                return Response<List<SMember>>.FromError(e.Message);
            }
        }
        public Response<List<SMember>> GetActiveMembers(string sessionID)
        {
            try
            {
